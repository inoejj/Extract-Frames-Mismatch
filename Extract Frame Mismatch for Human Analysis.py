...
dfRFvideo={}
RF_list={}
df_RF_list=list(df_Target['Video_no'].unique())
for x in df_RF_list:
    dfRFvideo[x] = df.loc[(df['Target'] == 1) & (df['Video_no']== x)]
...
for i in df_RF_list:
    ...
    vid_name=str(vid[-19:-4])+r"Human Prediction"
    ...
    while j < (len(RF_list[i])):
        ...
        for count in range(target_frame - 25, target_frame + 25):
            ...
            cv2.putText(im, 'Event ID: Human thinks is an attack', (100, 150), font, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
